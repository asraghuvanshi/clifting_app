...
class _OTPVerificationScreenState extends ConsumerState<OTPVerificationScreen> {
  ...
  Future<void> _verifyOTP() async {
    ...
    try {
      await ref
          .read(verifyResetOtpProvider.notifier)
          .verifyResetOtp(widget.email, otp);

      final state = ref.read(verifyResetOtpProvider);

      if (state.error == null && state.data?.success == true) {
        final resetToken = state.data?.data?.resetToken;

        showAppSnackBar(
          context,
          state.data?.message ?? 'OTP verified successfully',
          isError: false,
        );
        ...
        showAppSnackBar(
          context,
          state.error ?? state.data?.message ?? 'Verification failed',
          isError: true,
        );
      }
    } catch (error) {
  ...
  void _resendOTP() async {
    if (_resendCountdown == 0) {
      ...
      try {
        // Note: You'll need to call the appropriate API to resend OTP
        // Since I don't have the exact provider, I'm showing a placeholder
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(verifyResetOtpProvider);
